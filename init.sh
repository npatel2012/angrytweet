...
PROJECT="git@github.com/npatel2012/angrytweet"
PRODUCT="JBoss FUSE Service works + BPM Suite BAM"
AG_DEMO_HOME=/opt/angrytweet
...
AG_consumerKey="7SUTvXN6aPy0qDMsZxXbgzCjw"
AG_consumerSecret="85L4UjAd5fjtrYBuuE7XkjrwqRzSbRfKBhh3evnTWNmNHLHqcH"
AG_accessToken="3014551553-BrnP1nq58WmH9tOgOzSY4P5X5uxRq3B0tqaJolG"
AG_accessTokenSecret="wjaOcNtZVN90jMWZMg1kDC4n8vi1HZ3KW28vWsS3wEjaD"
AG_sinceId="3014551553"

# GMAIL SMTP Account configuration
AG_emailserverhost="smtps://smtp.gmail.com:465"
AG_emailserverusername="npatel.angryclaim@gmail.com"
AG_emailserverpassword="angryclaim"

# CRM location configuration
...
echo 'export AG_crm_host="'$AG_crm_host'"' >> ~/.bash_profile
echo 'export AG_crm_port="'$AG_crm_port'"' >> ~/.bash_profile
...
echo "verifying that MySQL/MariaDB is installed and started"
AG_FICHIER=/usr/share/java/mariadb-java-client-1.1.8.jar
...
service mariadb status ||  { echo "mysql server is not up.... aborting."; exit 1; }
echo
...
mysql -u root -h localhost -e"DROP USER 'jboss'@'localhost';"
...
mysql -u jboss -pjboss -h localhost -e"drop schema angrytweet;"
mysql -u jboss -pjboss -h localhost -e"create schema angrytweet;" || { echo "Error in creating required angrytweet schema. Please check and try again....."; exit 1; }
...
# create and copy mysql jar dependences files on FSW
   ...
   ln -s /usr/share/java/mariadb-java-client-1.1.8.jar $AG_FSW_HOME/modules/system/layers/base/com/mysql/main/mariadb-java-client-1.1.8.jar
...
# create and copy mysql jar dependences files on BAM
   ...
   ln -s /usr/share/java/mariadb-java-client-1.1.8.jar $AG_BAM_HOME/modules/system/layers/base/com/mysql/main/mariadb-java-client-1.1.8.jar
...
rm -f $AG_DEMO_HOME/FSW_Launch.sh # Remove FSW_Launch.sh file if already present
...
echo $AG_FSW_HOME/bin/standalone.sh -Djboss.bind.address=0.0.0.0 -Djboss.bind.address.management=0.0.0.0 >> $AG_DEMO_HOME/FSW_Launch.sh

rm -f $AG_DEMO_HOME/BAM_Launch.sh # Remove BAM_Launch.sh file if already present
echo "#!/bin/sh" >> $AG_DEMO_HOME/BAM_Launch.sh
echo "#Launch BPM-Suite BAM for Demo AngryClaim"  >> $AG_DEMO_HOME/BAM_Launch.sh
echo $AG_BAM_HOME/bin/standalone.sh -Djboss.bind.address=0.0.0.0 -Djboss.bind.address.management=0.0.0.0 >> $AG_DEMO_HOME/BAM_Launch.sh

chmod +x BAM_Launch.sh FSW_Launch.sh

rm -f $AG_DEMO_HOME/readme.txt
echo " " >>readme.txt
echo "******************************************************************">>readme.txt
echo "**                                                          ******">>readme.txt
